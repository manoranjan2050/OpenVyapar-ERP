  ...
  <div class="space-y-5">
    ...
    <div class="page-header animate-fade-down">
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{{ suppliers.length }} suppliers registered</p>
      </div>
    ...
    <div class="flex gap-3 animate-fade-up">
      <div class="relative flex-1 max-w-sm">
        ...
        <input v-model="search" type="text" placeholder="Search suppliers…" class="input pl-10" @input="debouncedFetch" />
      </div>
    </div>
      ...
      <table class="w-full">
        <thead>
          <tr class="border-b border-gray-100 dark:border-gray-800 bg-gray-50/80 dark:bg-gray-800/50">
            ...
            <th class="table-header text-center">Credit Days</th>
            <th class="table-header text-center">Status</th>
            <th class="table-header w-16"></th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-50 dark:divide-gray-800/50">
          <tr v-for="(s, i) in suppliers" :key="s.id" class="table-row group">
            <td class="table-cell">
            ...
            <td class="table-cell text-gray-600 dark:text-gray-400">{{ s.phone ?? '–' }}</td>
            <td class="table-cell font-mono text-xs text-gray-500">{{ s.gstin ?? '–' }}</td>
            <td class="table-cell text-gray-600 dark:text-gray-400">
              {{ [s.city, s.state].filter(Boolean).join(', ') || '–' }}
            </td>
            <td class="table-cell text-center">
              <span class="badge-purple">{{ s.credit_days ?? 0 }} days</span>
            </td>
            <td class="table-cell text-center">
              <span :class="s.is_active ? 'badge-green' : 'badge-gray'">
                <span class="w-1.5 h-1.5 rounded-full inline-block mr-1"
                      :class="s.is_active ? 'bg-emerald-500' : 'bg-gray-400'"></span>
                {{ s.is_active ? 'Active' : 'Inactive' }}
              </span>
            </td>
            <td class="table-cell text-right">
              <button @click="openForm(s)"
            ...
            <td colspan="7" class="py-16 text-center">
              <div class="flex flex-col items-center gap-3">
                <div class="w-14 h-14 rounded-2xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                  <TruckIcon class="w-7 h-7 text-gray-400" />
                </div>
                <p class="text-sm font-semibold text-gray-500">No suppliers found</p>
          ...
          <div class="relative card w-full max-w-lg p-6 shadow-2xl animate-scale-in">
            <div class="flex items-center justify-between mb-5">
              <div>
                ...
                <p class="text-sm text-gray-400 mt-0.5">Fill in supplier details</p>
              </div>
              <button @click="showForm = false" class="btn-icon"><XIcon class="w-4 h-4" /></button>
            </div>
            <form @submit.prevent="save" class="space-y-3">
              <div class="grid grid-cols-2 gap-3">
                <div class="col-span-2"><label class="label">Name *</label><input v-model="form.name" class="input" required placeholder="Supplier name" /></div>
                <div><label class="label">Phone</label><input v-model="form.phone" class="input" placeholder="+91 98765 43210" /></div>
                <div><label class="label">Email</label><input v-model="form.email" type="email" class="input" placeholder="email@supplier.com" /></div>
                <div><label class="label">GSTIN</label><input v-model="form.gstin" class="input font-mono" maxlength="15" placeholder="22AAAAA0000A1Z5" /></div>
                <div><label class="label">Credit Days</label><input v-model="form.credit_days" type="number" min="0" class="input" placeholder="30" /></div>
                <div><label class="label">City</label><input v-model="form.city" class="input" placeholder="Mumbai" /></div>
                <div><label class="label">State</label><input v-model="form.state" class="input" placeholder="Maharashtra" /></div>
                <div class="col-span-2"><label class="label">Address</label><textarea v-model="form.address" class="input resize-none" rows="2" placeholder="Full address" /></div>
              </div>
              <div v-if="formError" class="flex items-center gap-2 text-rose-600 text-sm bg-rose-50 dark:bg-rose-900/20 px-3 py-2 rounded-xl">
...
import { PlusIcon, SearchIcon, EditIcon, TruckIcon, XIcon, AlertCircleIcon } from 'lucide-vue-next'
...
const emptyForm = () => ({ name: '', phone: '', email: '', gstin: '', credit_days: 30, city: '', state: '', address: '' })
...
const debouncedFetch = useDebounceFn(() => loadSuppliers(), 300)
